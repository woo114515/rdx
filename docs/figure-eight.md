...
to `/cmd_vel_safe`. The requested limits are 0.66 m/s linear and 1.0 rad/s angular,
...
`task_timeout` defaults to 90 seconds and `cross_track_limit` defaults to 0.50 m;
both are ROS parameters and require low-speed field calibration before acceptance.
